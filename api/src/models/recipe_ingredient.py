"""
RecipeIngredient model - represents a specific quantity of a FoodItem in a Recipe.

This model connects Recipe to FoodItem with a serving quantity, allowing
recipes to specify exact amounts of each ingredient.
"""
...
class RecipeIngredient(Base):
    ...
    recipe_id = Column(SQLAlchemyUUID(as_uuid=True), ForeignKey("recipes.id", ondelete="CASCADE"), nullable=False)
    food_item_id = Column(SQLAlchemyUUID(as_uuid=True), ForeignKey("food_items.id"), nullable=False)
    servings = Column(DECIMAL(10, 2), nullable=False)  # Number of servings in recipe
    ...
    def total_protein_g(self) -> Optional[Decimal]:
        """Calculate total protein for this recipe ingredient."""
    ...
    def total_carbs_g(self) -> Optional[Decimal]:
        """Calculate total carbohydrates for this recipe ingredient."""
    ...
    def total_fat_g(self) -> Optional[Decimal]:
        """Calculate total fat for this recipe ingredient."""
    ...
    def total_cost(self) -> Optional[Decimal]:
        """Calculate total cost for this recipe ingredient."""
    ...
    def __repr__(self):
        return f"<RecipeIngredient(id={self.id}, recipe_id={self.recipe_id}, food_item_id={self.food_item_id}, servings={self.servings})>"